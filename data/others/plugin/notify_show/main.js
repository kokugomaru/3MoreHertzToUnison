(function(){
	...
	//CSSのpadding-left、padding-right、そしてimgタグのpadding-rightと紐づいてるのでCSS変更したらこっちも変更する
		...
		start:function (pm) {
			...
			if (pm.color       !== "") p_notify_show.color       = pm.color;
			if (pm.size   !== "") p_notify_show.size       = pm.size;
			if (pm.y  !== "") p_notify_show.y  = pm.y;
			if (pm.width     !== "") p_notify_show.width     = pm.width;
			if (pm.transition       !== "") p_notify_show.transition       = pm.transition;
			if (pm.tdecoration       !== "") p_notify_show.tdecoration       = pm.tdecoration;
			if (pm.bold       !== "") p_notify_show.bold       = pm.bold;
			...
			if (pm.thumsize       !== "") p_notify_show.thumsize       = pm.thumsize;
			if (pm.fold       !== "") p_notify_show.fold       = "./data/"+pm.fold+"/";
			if (pm.buf       !== "") p_notify_show.buf       = pm.buf;
			...
			if (pm.backcolor       !== "") p_notify_show.backcolor       = pm.backcolor;
			if (pm.opacity       !== "") p_notify_show.opacity       = pm.opacity;
			if (pm.delay       !== "") p_notify_show.delay       = pm.delay;
			if (pm.duration       !== "") p_notify_show.duration       = pm.duration;
			if (pm.showwait       !== "") p_notify_show.showwait       = pm.showwait;
			if (pm.shortcut       !== "") p_notify_show.shortcut       = pm.shortcut;
			if (pm.shortcutmag       !== "") p_notify_show.shortcutmag       = pm.shortcutmag;
			if(shownotifycnt==0){
